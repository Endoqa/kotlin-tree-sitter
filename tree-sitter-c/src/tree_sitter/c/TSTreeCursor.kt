...
public value class TSTreeCursor(
    ...
    public val context: NativeArray<uint32_t>
        get() = TSTreeCursor.contextHandle.invokeExact(this.`$mem`, 0L) as MemorySegment

    public constructor(gc: Boolean) : this(kotlin.run {
        require(gc) { "Do not call this if gc is not want" }
        Arena.ofAuto().allocate(TSTreeCursor.layout)
    })
    ...
    public companion object {
        ...
        public fun allocate(alloc: SegmentAllocator): TSTreeCursor =
            TSTreeCursor(alloc.allocate(TSTreeCursor.layout))
    }
}
