...
public value class TSRange(
    ...
    public var start_point: TSPoint
        get() = TSPoint(TSRange.start_pointHandle.invokeExact(this.`$mem`, 0L) as MemorySegment)
        set(`value`) {
            MemorySegment.copy(value.`$mem`, 0L, this.start_point.`$mem`, 0L, TSPoint.layout.byteSize())
        }

    public var end_point: TSPoint
        get() = TSPoint(TSRange.end_pointHandle.invokeExact(this.`$mem`, 0L) as MemorySegment)
        set(`value`) {
    ...
    public constructor(gc: Boolean) : this(kotlin.run {
        ...
        Arena.ofAuto().allocate(TSRange.layout)
    })
    ...
    public companion object {
        ...
        @JvmStatic
        public fun allocate(alloc: SegmentAllocator): TSRange = TSRange(alloc.allocate(TSRange.layout))
    }
}
