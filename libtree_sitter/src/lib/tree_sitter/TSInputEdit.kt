...
public value class TSInputEdit(
    ...
    public var old_end_byte: UInt
        get() = (old_end_byteHandle.get(this.`$mem`, 0L) as Int).toUInt()
        set(`value`) {
            old_end_byteHandle.set(this.`$mem`, 0L, value.toInt())
        }

    public var new_end_byte: UInt
        get() = (new_end_byteHandle.get(this.`$mem`, 0L) as Int).toUInt()
        set(`value`) {
            new_end_byteHandle.set(this.`$mem`, 0L, value.toInt())
        }
    ...
    public var old_end_point: TSPoint
        get() = TSPoint(old_end_pointHandle.invokeExact(this.`$mem`, 0L) as MemorySegment)
        set(`value`) {
            MemorySegment.copy(value.`$mem`, 0L, this.old_end_point.`$mem`, 0L, TSPoint.layout.byteSize())
        }

    public var new_end_point: TSPoint
        get() = TSPoint(new_end_pointHandle.invokeExact(this.`$mem`, 0L) as MemorySegment)
        set(`value`) {
